...
function moduleTools(){return `<div class="module-crud-tools"><button type="button" class="crud-add" onclick="editBiblicalGuide()">＋ Añadir</button><button type="button" class="crud-trash" onclick="openGuidesTrash()">Papelera <span>${trash.length}</span></button></div>`}
...
window.openBiblicalGuide=function(id,openedFromReader=false){const p=guides.find(x=>x.id===id);if(!p)return;$('#biblicalGuidesHome').classList.add('hidden');const d=$('#biblicalGuideDetail');d.classList.remove('hidden');d.innerHTML=`<div class="guide-detail-top"><button type="button" id="guideBackBtn" class="study-detail-back">← Guía</button>${openedFromReader?'<button type="button" class="return-to-reader-btn" style="appearance:none!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;width:auto!important;min-width:0!important;min-height:42px!important;margin-left:auto!important;padding:9px 14px!important;border:1px solid #9a792f!important;border-radius:14px!important;background:linear-gradient(180deg,#2b2c31 0%,#18191d 100%)!important;color:#f5efe4!important;-webkit-text-fill-color:#f5efe4!important;font:800 15px/1.15 Lora,serif!important;text-shadow:none!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.07),0 4px 12px rgba(0,0,0,.25)!important;white-space:nowrap!important;cursor:pointer!important;" onclick="window.returnBiblicalEntityToChapter&&window.returnBiblicalEntityToChapter()">Volver al capítulo</button>':''}<div class="detail-crud-actions"><button type="button" class="crud-block" onclick="openGuideBlockMenu('${esc(p.id)}')">Bloque</button><button type="button" class="crud-edit" onclick="editBiblicalGuide('${esc(p.id)}')">Editar</button><button type="button" class="crud-delete" onclick="deleteBiblicalGuide('${esc(p.id)}')">Eliminar</button></div></div><h1>${esc(plainTitle(p.title))}</h1><div class="guide-content">${formatText(p.content)}</div>`;bindGuidePopups();$('#guideBackBtn').onclick=()=>{d.classList.add('hidden');$('#biblicalGuidesHome').classList.remove('hidden');renderBiblicalGuides();window.scrollTo(0,0)};window.scrollTo(0,0)}
