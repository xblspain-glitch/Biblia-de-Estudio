...
self.addEventListener('activate', event => {
  event.waitUntil((async () => {
    ...
    for (const client of windows) {
      client.postMessage({ type: 'BIBLIA_UPDATE_READY', version: '3.1.33' });
    }
  })());
});

self.addEventListener('message', event => {
  if (event.data?.type === 'SKIP_WAITING' || event.data?.type === 'BIBLIA_FORCE_ACTIVATE') {
    self.skipWaiting();
  }
});
