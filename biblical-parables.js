...
async function ensureLoaded(){
 ...
 let defaults=[];try{defaults=await fetch('./biblical-parables.json?v=1.64.34',{cache:'no-store'}).then(r=>r.json())}catch(e){}
...
function bindParablePopups(){document.querySelectorAll('.parable-popup-trigger').forEach(btn=>btn.onclick=()=>showParablePopup(btn.dataset.popupTitle,btn.dataset.popupBody))}
window.showParablePopup=function(title,body){const wrap=document.createElement('div');wrap.className='parable-popup-overlay';wrap.innerHTML=`<div class="parable-popup-card"><div class="parable-popup-head"><h2>${esc(title)}</h2><button type="button" aria-label="Cerrar">✕</button></div><div class="parable-popup-body">${esc(body).replace(/\n/g,'<br>')}</div></div>`;document.body.appendChild(wrap);const close=()=>wrap.remove();wrap.querySelector('.parable-popup-head button').onclick=close;wrap.onclick=e=>{if(e.target===wrap)close()}}
