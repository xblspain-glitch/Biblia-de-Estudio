...
async function openRecognizedBiblicalEntity(entity){
  if(!entity)return;biblicalEntityReturnPosition=captureBiblicalEntityPosition();closeBiblicalEntityChooser();
  if(entity.type==='character'){
    try{await loadBiblicalCharactersV2252()}catch(_){toast('No se pudo abrir la ficha');return}
    await openBiblicalCharactersV2242();openBiblicalCharacterDetailV2242(entity.id);
  }else{
    await openBiblicalPlaces();openBiblicalPlaceDetail(entity.id);
  }
}
...
function showBiblicalPlaceOnMap(id){const x=biblicalPlacesData.find(e=>e.id===id);if(!x)return;toast('El mapa bíblico único se incorporará en una próxima actualización')}
...
function returnBiblicalEntityToChapter(){
  ...
  setTimeout(()=>{restoreBiblicalEntityPosition();biblicalEntityReturnPosition=null},220);
}
window.returnBiblicalEntityToChapter=returnBiblicalEntityToChapter;
function biblicalEntityReturnButton(){return biblicalEntityReturnPosition?'<button class="btn soft biblical-return-chapter" type="button" onclick="returnBiblicalEntityToChapter()">📖 Volver al capítulo</button>':''}
...
window.closeBiblicalCharactersV2242=function(){closeBiblicalCharactersOriginalV287?.();if(biblicalEntityReturnPosition){document.getElementById('readerScreen')?.classList.remove('hidden');document.getElementById('homeScreen')?.classList.add('hidden');restoreBiblicalEntityPosition();biblicalEntityReturnPosition=null}};
const closeBiblicalPlacesOriginalV287=window.closeBiblicalPlaces;
window.closeBiblicalPlaces=function(){closeBiblicalPlacesOriginalV287?.();if(biblicalEntityReturnPosition){document.getElementById('readerScreen')?.classList.remove('hidden');document.getElementById('homeScreen')?.classList.add('hidden');restoreBiblicalEntityPosition();biblicalEntityReturnPosition=null}};
