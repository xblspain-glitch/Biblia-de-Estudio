...
async function openRecognizedBiblicalEntity(entity){
  ...
  try{sessionStorage.setItem('biblicalEntityReturnStateV2816',JSON.stringify(biblicalEntityReturnPosition||{}))}catch(_){ }
  closeBiblicalEntityChooser();
  if(entity.type==='character'){
    ...
    openBiblicalCharacterDetailV2242(entity.id);
  }else{
...
function returnBiblicalEntityToChapter(){
  ...
  setTimeout(()=>{restoreBiblicalEntityPosition();biblicalEntityReturnPosition=null;window.__biblicalEntityOpenedFromReader=false;try{sessionStorage.removeItem('biblicalEntityReturnStateV2816')}catch(_){ }},220);
}
