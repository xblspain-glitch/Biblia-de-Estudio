...
async function init(){
  ...
  if('caches' in window){
    try{
      ...
      await Promise.all(cacheNames.filter(name=>name.startsWith('biblia-estudio-')&&name!=='biblia-estudio-v1.64.89').map(name=>caches.delete(name)));
