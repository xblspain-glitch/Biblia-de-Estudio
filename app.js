...
/* V2.8.0 · Lugares Bíblicos · arquitectura basada en Profecías */
const PLACES_CRUD_KEY='biblia_lugares_crud_v280';
...
function openBiblicalPlaceDetail(id){const x=biblicalPlacesData.find(e=>e.id===id);if(!x)return;activeBiblicalPlaceId=id;document.getElementById('biblicalPlacesHome')?.classList.add('hidden');const d=document.getElementById('biblicalPlaceDetail');if(!d)return;const hasCoords=Number.isFinite(x.coordinates.latitude)&&Number.isFinite(x.coordinates.longitude);d.innerHTML=`<div class="biblical-prophecy-detail-head"><button class="btn soft" type="button" onclick="closeBiblicalPlaceDetail()">← Volver</button></div><div class="prophecy-detail-tools"><button type="button" onclick="openBiblicalPlaceEditor('${escapeHtml(x.id)}')">Editar</button><button type="button" class="danger" onclick="deleteBiblicalPlace('${escapeHtml(x.id)}')">Eliminar</button></div><p class="biblical-prophecy-detail-theme">${escapeHtml(x.category)}</p><h1>${escapeHtml(x.name)}</h1><img class="biblical-prophecies-divider" src="separador_etiope_transparente_final.png?v=${APP_VERSION}" alt="" aria-hidden="true"><p class="place-current-region">${escapeHtml(x.currentRegion)}</p>${placeSection('PRIMERA APARICIÓN',`<p>${escapeHtml(x.firstAppearance||'Sin especificar')}</p>`,'biblical-prophecy-block place-reference')}${placeSection('DESCRIPCIÓN BREVE',placeParagraphs(x.shortDescription))}${placeSection('IMPORTANCIA BÍBLICA',placeParagraphs(x.biblicalImportance),'biblical-prophecy-importance')}${placeSection('HISTORIA',placeParagraphs(x.history))}${placeSection('PERSONAJES RELACIONADOS',placeList(x.relatedPeople))}${x.jesusRelation?placeSection('RELACIÓN CON JESÚS',placeParagraphs(x.jesusRelation)):''}${placeSection('VERSÍCULOS PRINCIPALES',placeList(x.mainVerses))}${placeSection('CURIOSIDADES',placeList(x.curiosities))}<section class="biblical-prophecy-explanation place-map-block"><h2>MAPA BÍBLICO</h2><p>${hasCoords?`${x.coordinates.latitude}, ${x.coordinates.longitude}`:'Coordenadas pendientes'}</p><button type="button" ${hasCoords?'':'disabled'} onclick="showBiblicalPlaceOnMap('${escapeHtml(x.id)}')">Mostrar en el mapa</button></section>`;d.classList.remove('hidden');window.scrollTo({top:0,behavior:'smooth'})}
