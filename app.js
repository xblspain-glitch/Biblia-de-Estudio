...
const PREMIUM_PLACE_MAPS={
  ...
  "jerusalen":{
    regionalOnly:true,
    regional:"mapa-jerusalen-region.webp",
    regionalText:"Jerusalén se encuentra en las colinas centrales de Judea. El mapa está recortado y calibrado específicamente para mostrar su posición junto a Betania, Emaús y Jericó."
  }
};
...
function premiumPlaceMapsBlock(place){
  ...
  if(maps.regionalOnly){
    return `<section class="place-geography-premium">
      <h2>SITUACIÓN GEOGRÁFICA</h2>
      <article class="place-geography-view place-geography-view-single">
        <div class="place-geography-copy">
          <h3>Ubicación en Judea</h3>
          <p>${escapeHtml(maps.regionalText)}</p>
        </div>
        <button type="button" class="place-map-image-button premium-map-open" data-map-src="${escapeHtml(maps.regional)}" data-map-title="${name} · Ubicación en Judea">
  ...
  return `<section class="place-geography-premium">
    ...
    <article class="place-geography-view">
      ...
      <button type="button" class="place-map-image-button premium-map-open" data-map-src="${escapeHtml(maps.general)}" data-map-title="${name} · Ubicación en el mundo bíblico">
    ...
    <article class="place-geography-view">
      ...
      <button type="button" class="place-map-image-button premium-map-open" data-map-src="${escapeHtml(maps.regional)}" data-map-title="${name} · Ubicación en la región">
        <img src="${escapeHtml(maps.regional)}?v=${APP_VERSION}" alt="Mapa regional con la ubicación de ${name}" loading="lazy">
      </button>
    </article>
  </section>`;
...
function handlePremiumMapOpen(event){
  const button=event.target.closest?.(".premium-map-open");
  if(!button)return;
  event.preventDefault();
  event.stopPropagation();
  const src=button.dataset.mapSrc;
  const title=button.dataset.mapTitle||"Mapa bíblico";
  if(src)openPremiumPlaceMap(src,title);
}
document.addEventListener("click",handlePremiumMapOpen);
document.addEventListener("keydown",event=>{
  if((event.key==="Enter"||event.key===" ")&&event.target.closest?.(".premium-map-open")){
    handlePremiumMapOpen(event);
  }
});
