...
function applyBiblicalPlacesMapTransform(){const svg=document.getElementById('placesMapSvg');if(!svg)return;svg.style.transform=`translate(${biblicalPlacesMapTranslateX}px,${biblicalPlacesMapTranslateY}px) scale(${biblicalPlacesMapScale})`}
function zoomBiblicalPlacesMap(factor){biblicalPlacesMapScale=Math.max(1,Math.min(3.5,biblicalPlacesMapScale*factor));applyBiblicalPlacesMapTransform()}
function resetBiblicalPlacesMap(){biblicalPlacesMapScale=1;biblicalPlacesMapTranslateX=0;biblicalPlacesMapTranslateY=0;applyBiblicalPlacesMapTransform()}
function selectBiblicalPlaceMapMarker(id){activeBiblicalMapPlaceId=id;renderBiblicalPlacesMap();showBiblicalPlacesMapCard(id)}
...
function focusBiblicalPlaceOnMap(id){const x=biblicalPlacesData.find(e=>e.id===id);if(!x||!biblicalMapHasCoords(x))return;activeBiblicalMapPlaceId=id;document.getElementById('placesMapSearch').value='';document.getElementById('placesMapCategory').value='';biblicalPlacesMapScale=2;const p=biblicalMapProject(x.coordinates.latitude,x.coordinates.longitude);biblicalPlacesMapTranslateX=(500-p.x)*.7;biblicalPlacesMapTranslateY=(280-p.y)*.7;renderBiblicalPlacesMap();showBiblicalPlacesMapCard(id)}
