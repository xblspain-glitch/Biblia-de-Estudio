...
async function resetCompletedBookProgressV3153(){
  ...
  const readings=completedBookReadingCountV3153(book),next=readings+1,nextBook=state.books[state.bookIndex+1]||null;
  ...
  const history=bookReadingHistoryV3153();history[book.key]=readings;localStorage.setItem(BOOK_READING_HISTORY_KEY_V3153,JSON.stringify(history));
  try{const manual=JSON.parse(localStorage.getItem('biblia_manual_book_readings_v3121')||'{}');if(manual&&typeof manual==='object'){delete manual[book.key];localStorage.setItem('biblia_manual_book_readings_v3121',JSON.stringify(manual))}}catch(_){}
  const now=Date.now();
  for(let chapter=1;chapter<=Number(book.chapters||0);chapter++){
...
function openBookCompletionDialogV3131(book){const dialog=document.getElementById('bookCompletionDialogV3131');if(!dialog||bookCompletionDialogOpenV3131)return;const copy=bookCompletionCopyV3131(book);const lastVerse=String(state.verses?.[state.verses.length-1]||'').trim();const reference=`${displayBook(book)} ${book.chapters}:${Math.max(1,state.verses?.length||1)}`;const nextBook=state.books[state.bookIndex+1]||null;document.getElementById('bookCompletionEyebrowV3131').textContent=copy.eyebrow;document.getElementById('bookCompletionTitleV3131').textContent=copy.title;document.getElementById('bookCompletionMessageV3131').textContent=copy.message;document.getElementById('bookCompletionVerseV3131').textContent=lastVerse;document.getElementById('bookCompletionReferenceV3131').textContent=reference;document.getElementById('bookCompletionChaptersV3131').textContent=`${book.chapters} ${Number(book.chapters)===1?'capítulo completado':'capítulos completados'}`;const continueButton=document.getElementById('bookCompletionContinueV3131');if(nextBook){continueButton.hidden=false;continueButton.textContent=`Continuar con ${displayBook(nextBook)}`}else continueButton.hidden=true;
...
const BACKUP_KEYS=[
  ...
  CHAPTER_READING_PROGRESS_KEY_V316,BOOK_READING_HISTORY_KEY_V3153,BOOK_READING_OVERRIDES_KEY_V3154,
