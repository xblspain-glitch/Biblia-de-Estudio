...
async function init(){
  ...
  if('caches' in window){
    try{
      ...
      await Promise.all(cacheNames.filter(name=>name.startsWith('biblia-estudio-')&&name!=='biblia-estudio-v1.47.0').map(name=>caches.delete(name)));
...
function updateSelection(){
  ...
  if(hasSelection){
    ...
    if(favoriteBtn){favoriteBtn.dataset.mode=allSaved?'remove':'save';favoriteBtn.setAttribute('aria-label',allSaved?'Quitar de guardados':'Guardar');favoriteBtn.title=allSaved?'Quitar de guardados':'Guardar'}
  }else if(favoriteBtn){
    favoriteBtn.dataset.mode='save';favoriteBtn.setAttribute('aria-label','Guardar');favoriteBtn.title='Guardar';
  }
}
