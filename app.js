...
const PREMIUM_PLACE_MAPS={"jerusalen":{"latitude":31.78,"longitude":35.23,"generalText":"Jerusalén se encuentra en Judea, dentro del corazón de la tierra de Israel.","regionalText":"El marcador se calcula directamente desde las coordenadas de Jerusalén."},"belen":{"latitude":31.705,"longitude":35.202,"generalText":"Belén se encuentra en Judea, al sur de Jerusalén.","regionalText":"Belén está situada a unos 8 km al sur de Jerusalén."},"belen-de-juda":{"latitude":31.705,"longitude":35.202,"generalText":"Belén de Judá se encuentra en Judea, al sur de Jerusalén.","regionalText":"Belén de Judá corresponde a la misma ciudad situada al sur de Jerusalén."},"betania":{"latitude":31.77,"longitude":35.26,"generalText":"Betania se encontraba en Judea, inmediatamente al este de Jerusalén.","regionalText":"Betania estaba al este de Jerusalén, junto al monte de los Olivos."},"jerico":{"latitude":31.87,"longitude":35.44,"generalText":"Jericó se encuentra al este de Judea, junto al valle del Jordán.","regionalText":"Jericó está situada al noroeste del mar Muerto."},"hebron":{"latitude":31.53,"longitude":35.1,"generalText":"Hebrón se encuentra en la zona montañosa del sur de Judea.","regionalText":"Hebrón se encuentra al sur de Belén, en las montañas de Judea."},"monte-de-los-olivos":{"latitude":31.78,"longitude":35.25,"generalText":"El monte de los Olivos se levanta al este de Jerusalén, dentro de Judea.","regionalText":"El monte se encuentra inmediatamente al este de Jerusalén."}};
const PREMIUM_JUDEA_BOUNDS={minLon:34.80,maxLon:35.65,minLat:31.15,maxLat:32.10};

function projectPremiumCoordinates(latitude,longitude){
  const b=PREMIUM_JUDEA_BOUNDS;
  return {
    x:(longitude-b.minLon)/(b.maxLon-b.minLon)*100,
    y:(b.maxLat-latitude)/(b.maxLat-b.minLat)*100
  };
}

function premiumRegionalMarker(maps,name){
  const pos=projectPremiumCoordinates(maps.latitude,maps.longitude);
  return `<span class="premium-map-marker" style="left:${pos.x}%;top:${pos.y}%" aria-hidden="true"><span class="premium-map-pin"></span><span class="premium-map-label">${escapeHtml(name)}</span></span>`;
}
...
function premiumPlaceMapsBlock(place){
  ...
  const pos=projectPremiumCoordinates(maps.latitude,maps.longitude);
  const general="mapa-maestro-mundo-biblico.webp";
  const regional="mapa-maestro-judea.webp";
  return `<section class="place-geography-premium">
    ...
    <article class="place-geography-view">
      ...
      <button type="button" class="place-map-image-button" onclick="openPremiumPlaceMap('${general}','Judea · Ubicación en el mundo bíblico',null,null,'')">
        <span class="premium-map-stage">
          <img src="${general}?v=${APP_VERSION}" alt="Mapa general con Judea destacada" loading="lazy">
        </span>
      </button>
    </article>
    <article class="place-geography-view">
      ...
      <button type="button" class="place-map-image-button" onclick="openPremiumPlaceMap('${regional}','${escapeHtml(name)} · Ubicación en Judea',${pos.x},${pos.y},'${escapeHtml(name)}')">
        <span class="premium-map-stage">
          <img src="${regional}?v=${APP_VERSION}" alt="Mapa de Judea con la ubicación de ${escapeHtml(name)}" loading="lazy">
          ${premiumRegionalMarker(maps,name)}
        </span>
      </button>
    </article>
  </section>`;
...
function openPremiumPlaceMap(src,title,markerX=50,markerY=50,markerLabel=''){
  ...
  if(modalMarker){
    ...
    modalMarker.style.display=hasMarker?"flex":"none";
    if(hasMarker){
      modalMarker.style.left=markerX+"%";
      modalMarker.style.top=markerY+"%";
      const modalLabel=modalMarker.querySelector(".premium-map-label");
      modalLabel.textContent=markerLabel||"";
      modalLabel.style.display=markerLabel?"inline-block":"none";
    }
  }
