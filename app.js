...
/* V2.4.0 · Profecías mesiánicas revisadas */
...
async function loadBiblicalProphecies(){
  if(biblicalPropheciesLoaded)return biblicalPropheciesData;
  try{
    const response=await fetch(freshUrl('biblical-prophecies-v240.json'));
    if(!response.ok)throw new Error('No se pudo cargar Profecías');
    biblicalPropheciesData=await response.json();
  }catch(error){
    console.error(error);
    biblicalPropheciesData=[];
  }
...
async function openBiblicalProphecies(){
  ...
  await loadBiblicalProphecies();
  renderBiblicalProphecies();
  window.scrollTo({top:0,behavior:'smooth'});
}
function renderBiblicalProphecies(){
  const list=document.getElementById('biblicalPropheciesList');
  if(!list)return;
  const query=normalizeProphecyText(document.getElementById('biblicalPropheciesSearch')?.value);
  const filtered=biblicalPropheciesData.filter(item=>{
    const haystack=normalizeProphecyText([item.title,item.prophecyRef,item.fulfillmentRef,item.prophecy,item.fulfillment,item.explanation,item.importance,item.theme].join(' '));
    return !query||haystack.includes(query);
  });
  const count=document.getElementById('biblicalPropheciesCount');
  if(count)count.textContent=`${filtered.length} ${filtered.length===1?'profecía':'profecías'}`;
  list.innerHTML=filtered.length?filtered.map(item=>`
    <button type="button" class="biblical-prophecy-card" onclick="openBiblicalProphecyDetail('${escapeHtml(item.id)}')">
      <span class="biblical-prophecy-theme">${escapeHtml(item.theme||'Profecía mesiánica')}</span>
      <strong>${escapeHtml(item.title)}</strong>
      <span class="biblical-prophecy-refs">${escapeHtml(item.prophecyRef)} · ${escapeHtml(item.fulfillmentRef)}</span>
    </button>`).join(''):`<p class="biblical-prophecies-empty">No se han encontrado profecías con esa búsqueda.</p>`;
}
function highlightProphecyText(text,phrases){
  const source=String(text||'');
  const wanted=(Array.isArray(phrases)?phrases:[]).filter(Boolean);
  if(!wanted.length)return escapeHtml(source);
  const lower=source.toLocaleLowerCase('es');
  const ranges=[];
  wanted.forEach(phrase=>{
    const target=String(phrase).toLocaleLowerCase('es');
    let start=0,index;
    while(target&&(index=lower.indexOf(target,start))!==-1){ranges.push([index,index+target.length]);start=index+target.length;}
  });
  ...
  let html='',cursor=0;
  merged.forEach(([start,end])=>{html+=escapeHtml(source.slice(cursor,start));html+='<mark class="prophecy-highlight">'+escapeHtml(source.slice(start,end))+'</mark>';cursor=end;});
  return html+escapeHtml(source.slice(cursor));
}
function prophecyParagraphs(text){
  return String(text||'').split(/\n\s*\n/).filter(Boolean).map(paragraph=>`<p>${escapeHtml(paragraph)}</p>`).join('');
}
function openBiblicalProphecyDetail(id){
  const item=biblicalPropheciesData.find(entry=>entry.id===id);
  if(!item)return;
  document.getElementById('biblicalPropheciesHome')?.classList.add('hidden');
  const detail=document.getElementById('biblicalProphecyDetail');
  if(!detail)return;
  detail.innerHTML=`
    <div class="biblical-prophecy-detail-head"><button class="btn soft" type="button" onclick="closeBiblicalProphecyDetail()">← Volver</button></div>
    <p class="biblical-prophecy-detail-theme">${escapeHtml(item.theme||'Profecía mesiánica')}</p>
    ...
    <section class="biblical-prophecy-block prophecy-original">
      <h2>PROFECÍA</h2>
      <h3>${escapeHtml(item.prophecyRef)}</h3>
      <blockquote>${highlightProphecyText(item.prophecy,item.prophecyHighlights)}</blockquote>
    </section>
    <section class="biblical-prophecy-block prophecy-fulfilled">
      <h2>CUMPLIMIENTO</h2>
      <h3>${escapeHtml(item.fulfillmentRef)}</h3>
      <blockquote>${highlightProphecyText(item.fulfillment,item.fulfillmentHighlights)}</blockquote>
    </section>
    <section class="biblical-prophecy-explanation">
      <h2>EXPLICACIÓN</h2>
      ${prophecyParagraphs(item.explanation)}
    </section>
    <section class="biblical-prophecy-importance">
      <h2>IMPORTANCIA</h2>
...
function closeBiblicalProphecyDetail(){
  document.getElementById('biblicalProphecyDetail')?.classList.add('hidden');
  document.getElementById('biblicalPropheciesHome')?.classList.remove('hidden');
  window.scrollTo({top:0,behavior:'smooth'});
}
