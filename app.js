...
function updateChapterReadingProgressV316(options={}){
  ...
  const next={read:completed?total:read,total,completed,updatedAt:Date.now()};
  chapterReadingProgressV316[key]=next;
  setChapterRingV316(next.read,total,next.completed);
  if(next.read!==previous.read||next.completed!==previous.completed||Number(previous.total)!==total){
