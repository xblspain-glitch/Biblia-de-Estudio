...
/* V3.1.0 · Más y Calendario de festividades */
...
function biblicalFestivityStore(){
  try{return JSON.parse(localStorage.getItem('biblia_festividades_v310')||'{}')}catch(_){return{}}
}
function saveBiblicalFestivityStore(store){
  localStorage.setItem('biblia_festividades_v310',JSON.stringify(store||{}));
}
function mergedBiblicalFestivity(item){
  const custom=biblicalFestivityStore()[item.id]||{};
  return{
    ...item,
    notes:custom.notes??item.notes??'',
    passageTexts:{...(item.passageTexts||{}),...(custom.passageTexts||{})}
  };
}
...
function closeBiblicalFestivityDialog(){document.getElementById('biblicalFestivityDialog')?.close()}
function backToBiblicalFestivityList(){
  currentBiblicalFestivityId=null;
  document.getElementById('biblicalFestivityListView')?.classList.remove('hidden');
  document.getElementById('biblicalFestivityDetailView')?.classList.add('hidden');
...
async function renderBiblicalFestivityLibrary(){
  ...
  const rows=biblicalFestivitiesData.filter(f=>!q||normalizeText(`${f.title} ${f.date} ${f.summary}`).includes(q));
  box.innerHTML=rows.map(f=>`<button class="biblical-festivity-row" type="button" onclick="openBiblicalFestivityDetail('${f.id}')">
    <strong>${escapeHtml(f.title)}</strong><span>${escapeHtml(f.date)}</span>
...
async function openBiblicalFestivityDetail(id){
  await loadBiblicalFestivities();
  const base=biblicalFestivitiesData.find(f=>f.id===id);
  if(!base){openBiblicalFestivityLibrary();return}
  const f=mergedBiblicalFestivity(base);
  currentBiblicalFestivityId=id;
  const dialog=document.getElementById('biblicalFestivityDialog');
  if(dialog&&!dialog.open)dialog.showModal();
  document.getElementById('biblicalFestivityListView')?.classList.add('hidden');
  document.getElementById('biblicalFestivityDetailView')?.classList.remove('hidden');
  const box=document.getElementById('biblicalFestivityDetail');
  const passages=(f.passages||[]).map((ref,i)=>`<section class="biblical-festivity-passage">
    <h3>${escapeHtml(ref)}</h3>
    <textarea id="biblicalFestivityPassage${i}" data-ref="${escapeHtml(ref)}" placeholder="Texto bíblico...">${escapeHtml(f.passageTexts?.[ref]||'')}</textarea>
  </section>`).join('');
  box.innerHTML=`<header class="biblical-festivity-detail-head">
      <h2>${escapeHtml(f.title)}</h2><p>${escapeHtml(f.date)}</p>
    </header>
    <section class="biblical-festivity-section"><h3>Explicación</h3><p>${escapeHtml(f.summary)}</p></section>
    <section class="biblical-festivity-section"><h3>Significado</h3><p>${escapeHtml(f.meaning)}</p></section>
    <section class="biblical-festivity-section"><h3>Notas</h3>
      <textarea id="biblicalFestivityNotes" placeholder="Escribe tus notas...">${escapeHtml(f.notes||'')}</textarea>
    </section>
    <section class="biblical-festivity-section"><h3>Pasajes bíblicos</h3>${passages}</section>
    <button class="wide-btn biblical-festivity-save" type="button" onclick="saveBiblicalFestivityChanges()">Guardar cambios</button>`;
}
function saveBiblicalFestivityChanges(){
  if(!currentBiblicalFestivityId)return;
  const passageTexts={};
  document.querySelectorAll('#biblicalFestivityDetail textarea[data-ref]').forEach(el=>passageTexts[el.dataset.ref]=el.value);
  const store=biblicalFestivityStore();
  store[currentBiblicalFestivityId]={
    notes:document.getElementById('biblicalFestivityNotes')?.value||'',
    passageTexts
  };
  saveBiblicalFestivityStore(store);
  toast('Festividad guardada');
}
