...
async function loadBiblicalProphecies(){
  ...
  try{
    const response=await fetch(freshUrl('biblical-prophecies-v232.json'));
...
function renderBiblicalProphecies(){
  ...
  const filtered=biblicalPropheciesData.filter(item=>{
    const haystack=normalizeProphecyText([item.title,item.prophecyRef,item.fulfillmentRef,item.prophecy,item.fulfillment,item.explanation,item.theme].join(' '));
...
function openBiblicalProphecyDetail(id){
  ...
  detail.innerHTML=`
    ...
    <section class="biblical-prophecy-block prophecy-original">
      ...
      <blockquote>${escapeHtml(item.prophecy)}</blockquote>
    </section>
    <section class="biblical-prophecy-block prophecy-fulfilled">
      <h2>CUMPLIMIENTO</h2>
      <h3>${escapeHtml(item.fulfillmentRef)}</h3>
      <blockquote>${escapeHtml(item.fulfillment)}</blockquote>
    </section>
