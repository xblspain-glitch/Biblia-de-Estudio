...
function updateChapterReadingProgressV316(options={}){
  ...
  const book=state.books?.[state.bookIndex];
  const isLastChapter=book&&Number(state.chapter)===Number(book.chapters);
  if(reachedEnd&&isLastChapter&&!bookCompletionShownForViewV3132){
    bookCompletionShownForViewV3132=true;
    maybeShowBookCompletionV3131();
  }
}


let bookCompletionDialogOpenV3131=false;
let bookCompletionShownForViewV3132=false;
...
function maybeShowBookCompletionV3131(){const book=state.books?.[state.bookIndex];if(!book)return;if(Number(state.chapter)!==Number(book.chapters))return;setTimeout(()=>openBookCompletionDialogV3131(book),180)}
...
function initChapterReadingProgressV316(){
  bookCompletionShownForViewV3132=false;
  const key=chapterReadingKeyV316();
  const total=state.verses.length;
