...
async function init(){
  ...
  if('caches' in window){
    try{
      ...
      await Promise.all(cacheNames.filter(name=>name.startsWith('biblia-estudio-')&&name!=='biblia-estudio-v1.58.0').map(name=>caches.delete(name)));
  ...
  state.baseTitles=(window.BUILTIN_TITLES&&typeof window.BUILTIN_TITLES==='object')?JSON.parse(JSON.stringify(window.BUILTIN_TITLES)):{};
  if(!countTitleLayer(state.baseTitles)){
...
const HEADER_SCHEDULE=[
  {from:6,to:9,file:'cabecera-1.png',label:'amanecer'},
  {from:9,to:17,file:'cabecera-2.png',label:'día'},
  {from:17,to:20,file:'cabecera-3.png',label:'atardecer'},
  {from:20,to:30,file:'cabecera-4.png',label:'noche'}
];
...
function applyTimeHeader(){
  const image=$('#timeHeaderImage');if(!image)return;
...
function getSavedStats(){
  ...
  const now=new Date(),todayKey=`${now.getFullYear()}-${now.getMonth()+1}-${now.getDate()}`;
  const dayKey=t=>{const d=new Date(t);return Number.isFinite(d.getTime())?`${d.getFullYear()}-${d.getMonth()+1}-${d.getDate()}`:''};
  const dated=entries.filter(x=>x.savedAt>0),today=dated.filter(x=>dayKey(x.savedAt)===todayKey).length;
  return{entries,dated,today,days:new Set(dated.map(x=>dayKey(x.savedAt))).size,last:dated[0]||entries[0]||null};
}
...
function renderStats(){
  const st=getSavedStats(),now=new Date();
  ...
  $('#statsSavedToday').textContent=st.today.toLocaleString('es-ES');
  $('#statsDaysActive').textContent=st.days.toLocaleString('es-ES');
  $('#statsTitlesTotal').textContent=countTitleLayer(state.baseTitles||{}).toLocaleString('es-ES');
  $('#statsLastSaved').innerHTML=st.last?`<strong>${escapeHtml(st.last.ref||'Versículo guardado')}</strong><span>${escapeHtml(formatSavedDate(st.last.savedAt))}</span>`:'Todavía no hay versículos guardados.';
  $('#statsRecent').innerHTML=st.entries.length?st.entries.slice(0,20).map(x=>`<div class="stats-recent-row"><strong>${escapeHtml(x.ref||'Versículo guardado')}</strong><span>${escapeHtml(formatSavedDate(x.savedAt))}</span></div>`).join(''):'<p class="empty-saved">Todavía no hay versículos guardados.</p>';
  updateHomeStatsSummary();
}
...
function wireHomeActions(){
  ...
  $('#homeStats')?.addEventListener('click',openStats);
}
wireHomeActions();
