...
/* V3.1.15 · Mi camino en la Palabra */
...
function renderMyWordJourneyV3110(){
  ...
  container.innerHTML=`
    ...
    <section class="word-journey-section">
      <h2>Mapa del progreso</h2>
      <div id="wordJourneyBookMapV3115" class="word-journey-book-map"></div>
    </section>
  ...
  renderWordJourneyBookMapV3115();
}
}


function wordJourneyBookProgressV3115(book){
  const total=Math.max(0,Number(book?.chapters)||0);
  if(!total)return{status:'not-started',completed:0,inProgress:0,total:0,percent:0};
  ...
  for(let chapter=1;chapter<=total;chapter++){
    ...
    const item=chapterReadingProgressV316?.[key];
    if(item?.completed){
      completed+=1;
  ...
  return{
    ...
    inProgress,
    total,
    percent:Math.max(0,Math.min(100,completed/Math.max(1,total)*100))
  };
}

function wordJourneyBookStatusLabelV3115(status){
  if(status==='completed')return'Completado';
  if(status==='in-progress')return'En curso';
  return'Sin empezar';
}

function wordJourneyBookCardV3115(book){
  const progress=wordJourneyBookProgressV3115(book);
  const name=displayBook(book);
  return `<button class="word-journey-book-card ${progress.status}" type="button"
    onclick="openWordJourneyBookDetailV3115('${wordJourneyEscapeV3110(book.key)}')">
    <div class="word-journey-book-card-head">
      <strong>${wordJourneyEscapeV3110(name)}</strong>
      <span>${wordJourneyEscapeV3110(wordJourneyBookStatusLabelV3115(progress.status))}</span>
    </div>
    <div class="word-journey-book-card-meta">
      ${progress.completed} de ${progress.total} capítulos
    </div>
    <div class="word-journey-book-card-progress">
      <span style="width:${progress.percent}%"></span>
    </div>
  </button>`;
}

function renderWordJourneyBookMapV3115(){
  const container=document.getElementById('wordJourneyBookMapV3115');
  if(!container)return;
  ...
  const all=books.map(wordJourneyBookProgressV3115);
  ...
  const notStarted=all.filter(item=>item.status==='not-started').length;

  container.innerHTML=`
    <div class="word-journey-book-summary">
    ...
    <div class="word-journey-testament">
      <h3>Antiguo Testamento</h3>
      <div class="word-journey-book-grid">
        ${ot.map(wordJourneyBookCardV3115).join('')}
      </div>
    </div>
    ...
    <div class="word-journey-testament">
      ...
      <div class="word-journey-book-grid">
        ${nt.map(wordJourneyBookCardV3115).join('')}
      </div>
    </div>

    <div id="wordJourneyBookDetailV3115" class="word-journey-book-detail hidden"></div>`;
}

function openWordJourneyBookDetailV3115(bookKey){
  const book=(state.books||[]).find(item=>item.key===bookKey);
  const box=document.getElementById('wordJourneyBookDetailV3115');
  if(!book||!box)return;

  const progress=wordJourneyBookProgressV3115(book);
  box.classList.remove('hidden');
  box.innerHTML=`
    <div class="word-journey-book-detail-card">
      <button class="word-journey-book-detail-close" type="button"
        onclick="closeWordJourneyBookDetailV3115()" aria-label="Cerrar">×</button>
      <h3>${wordJourneyEscapeV3110(displayBook(book))}</h3>
      <p>${wordJourneyEscapeV3110(wordJourneyBookStatusLabelV3115(progress.status))}</p>
      <div class="word-journey-book-detail-stats">
        <span><strong>${progress.completed}</strong> capítulos completados</span>
        <span><strong>${progress.inProgress}</strong> capítulos en curso</span>
        <span><strong>${progress.total-progress.completed-progress.inProgress}</strong> sin empezar</span>
      </div>
      <div class="word-journey-book-card-progress large">
        <span style="width:${progress.percent}%"></span>
      </div>
    </div>`;
}

function closeWordJourneyBookDetailV3115(){
  document.getElementById('wordJourneyBookDetailV3115')?.classList.add('hidden');
}

window.openWordJourneyBookDetailV3115=openWordJourneyBookDetailV3115;
window.closeWordJourneyBookDetailV3115=closeWordJourneyBookDetailV3115;
...
function openMoreFunctions(){
  /* V3.1.15: el punto de lectura se captura exactamente al entrar en Más. */
...
/* V3.1.15 · Aviso de festividad al abrir la aplicación. */
...
/* V3.1.15: el aviso de festividad se muestra después de pulsar Entrar. */
