...
async function init(){
  ...
  if('caches' in window){
    try{
      ...
      await Promise.all(cacheNames.filter(name=>name.startsWith('biblia-estudio-')&&name!=='biblia-estudio-v1.44.0').map(name=>caches.delete(name)));
  ...
  showHome();
  updateReadingPointUI();
  requestAnimationFrame(()=>{prepareBooksDrawer();updateReadingPointUI()});
...
function updateSelection(){
  ...
  if(hasSelection){
    ...
    favoriteBtn.innerHTML='<span>🔖</span>Guardar';favoriteBtn.dataset.mode='save';
  }
}
