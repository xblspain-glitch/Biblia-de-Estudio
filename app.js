...
const state={books:[],bookIndex:0,chapter:1,verses:[],titles:{},selected:new Set(),highlights:JSON.parse(localStorage.getItem('highlights')||'{}'),favorites:JSON.parse(localStorage.getItem('favorites')||'{}'),explanations:JSON.parse(localStorage.getItem('explanations')||'{}'),fragmentClarifications:JSON.parse(localStorage.getItem(FRAGMENT_CLARIFICATIONS_KEY_V3168)||'{}'),readingPoints:Array.isArray(storedReadingPoints)?storedReadingPoints.map((p,i)=>({...p,id:String(p.id||`${Date.now()}-${i}-${Math.random().toString(36).slice(2)}`)})):[],importedTitles:JSON.parse(localStorage.getItem('importedTitles')||'{}'),externalBible:null,baseTitles:{},dictionaryBase:[],dictionaryCustom:JSON.parse(localStorage.getItem('dictionaryCustom')||'[]'),dictionaryEdits:JSON.parse(localStorage.getItem('dictionaryEdits')||'{}'),dictionaryDeleted:JSON.parse(localStorage.getItem('dictionaryDeleted')||'[]'),activeReadingPoint:JSON.parse(localStorage.getItem('activeReadingPoint')||'null'),lastReadingPoint:JSON.parse(localStorage.getItem('lastReadingPoint')||'null')};
...
async function runSearch(){
  ...
  for(let bi=0;bi<state.books.length&&verseResults.length<100;bi++){
    ...
    for(let ci=0;ci<chapters.length&&verseResults.length<100;ci++)for(let vi=0;vi<chapters[ci].length&&verseResults.length<100;vi++){
      const text=limpiarTextoBiblico(chapters[ci][vi]),equivalents=activeEquivalencesByVerse.get(`${b.key}:${ci+1}:${vi+1}`)||'';
...
const BACKUP_KEYS=[
  // Lectura y estudio
  'highlights','favorites','explanations',FRAGMENT_CLARIFICATIONS_KEY_V3168,'last','readingPoints','readingPoint','activeReadingPoint','lastReadingPoint',
