...
reader.addEventListener('pointerdown',e=>{
  ...
  wordPressTimer=setTimeout(()=>{
    ...
    cancelWordPress();
    if(clean){recognizeBiblicalEntityOrDictionary(clean,word).then(found=>{if(!found){openDictionary(clean);toast(`Buscando “${clean}” en el diccionario`)}})}
  },650);
},{passive:true});
...
let biblicalPlacesMapScale=1,biblicalPlacesMapTranslateX=0,biblicalPlacesMapTranslateY=0,activeBiblicalMapPlaceId='';
let biblicalPlacesMapPointers=new Map(),biblicalPlacesMapGesture=null,biblicalPlacesMapSuppressClickUntil=0,biblicalPlacesMapLastTap=null,biblicalPlacesMapEventsReady=false;
function biblicalMapHasCoords(x){return Number.isFinite(x?.coordinates?.latitude)&&Number.isFinite(x?.coordinates?.longitude)}
function biblicalMapProject(lat,lon){const minLon=11,maxLon=48,minLat=25,maxLat=43;return{x:45+((lon-minLon)/(maxLon-minLon))*910,y:520-((lat-minLat)/(maxLat-minLat))*465}}
...
async function openBiblicalPlacesMap(focusId=''){document.getElementById('biblicalPlacesHome')?.classList.add('hidden');document.getElementById('biblicalPlaceDetail')?.classList.add('hidden');document.getElementById('biblicalPlacesMap')?.classList.remove('hidden');await loadBiblicalPlaces();populateBiblicalPlacesMapCategories();initBiblicalPlacesMapGestures();activeBiblicalMapPlaceId=focusId||'';if(!focusId)resetBiblicalPlacesMap(false);renderBiblicalPlacesMap();if(focusId)setTimeout(()=>focusBiblicalPlaceOnMap(focusId),40);window.scrollTo({top:0,behavior:'smooth'})}
function closeBiblicalPlacesMap(){document.getElementById('biblicalPlacesMap')?.classList.add('hidden');if(activeBiblicalPlaceId){document.getElementById('biblicalPlaceDetail')?.classList.remove('hidden')}else{document.getElementById('biblicalPlacesHome')?.classList.remove('hidden')}activeBiblicalMapPlaceId='';window.scrollTo({top:0,behavior:'smooth'})}
function renderBiblicalPlacesMap(){const markerBox=document.getElementById('placesMapMarkers'),grid=document.getElementById('placesMapGrid');if(!markerBox)return;const q=normalizePlaceText(document.getElementById('placesMapSearch')?.value||''),cat=document.getElementById('placesMapCategory')?.value||'';const all=biblicalPlacesData.filter(biblicalMapHasCoords);const filtered=all.filter(x=>(!cat||x.category===cat)&&(!q||normalizePlaceText([x.name,x.currentRegion,x.category].join(' ')).includes(q)));document.getElementById('placesMapCount').textContent=`${filtered.length} marcadores · ${biblicalPlacesData.length-all.length} pendientes`;if(grid&&!grid.childElementCount){let lines='';for(let lon=15;lon<=45;lon+=5){const p=biblicalMapProject(25,lon);lines+=`<line x1="${p.x}" y1="40" x2="${p.x}" y2="530"/><text x="${p.x+4}" y="548">${lon}°E</text>`}for(let lat=27;lat<=42;lat+=3){const p=biblicalMapProject(lat,11);lines+=`<line x1="35" y1="${p.y}" x2="970" y2="${p.y}"/><text x="4" y="${p.y-4}">${lat}°N</text>`}grid.innerHTML=lines}markerBox.innerHTML=filtered.map(x=>{const p=biblicalMapProject(x.coordinates.latitude,x.coordinates.longitude),active=x.id===activeBiblicalMapPlaceId;return `<g class="places-map-marker${active?' active':''}" data-place-id="${escapeHtml(x.id)}" transform="translate(${p.x} ${p.y})" onclick="selectBiblicalPlaceMapMarker('${escapeHtml(x.id)}')"><circle r="${active?10:7}"/><circle class="marker-ring" r="${active?17:13}"/><title>${escapeHtml(x.name)}</title></g>`}).join('');applyBiblicalPlacesMapTransform();if(activeBiblicalMapPlaceId)showBiblicalPlacesMapCard(activeBiblicalMapPlaceId)}
function biblicalPlacesMapClamp(){
  const wrap=document.getElementById('placesMapCanvasWrap');if(!wrap)return;
  const w=wrap.clientWidth,h=wrap.clientHeight,scaledW=w*biblicalPlacesMapScale,scaledH=h*biblicalPlacesMapScale;
  const margin=Math.min(90,Math.min(w,h)*.2);
  biblicalPlacesMapTranslateX=Math.min(margin,Math.max(w-scaledW-margin,biblicalPlacesMapTranslateX));
  biblicalPlacesMapTranslateY=Math.min(margin,Math.max(h-scaledH-margin,biblicalPlacesMapTranslateY));
}
function updateBiblicalPlacesMapReadableScale(){
  const s=biblicalPlacesMapScale||1;
  document.querySelectorAll('#placesMapMarkers .places-map-marker').forEach(g=>{
    const active=g.classList.contains('active'),circles=g.querySelectorAll('circle');
    if(circles[0]){circles[0].setAttribute('r',(active?10:7)/s);circles[0].style.strokeWidth=(2/s)+'px'}
    if(circles[1]){circles[1].setAttribute('r',(active?17:13)/s);circles[1].style.strokeWidth=((active?3:2)/s)+'px'}
  });
  document.querySelectorAll('.places-map-region-labels text').forEach(t=>t.style.fontSize=(15/s)+'px');
  document.querySelectorAll('#placesMapGrid text').forEach(t=>t.style.fontSize=(11/s)+'px');
  document.querySelectorAll('#placesMapGrid line').forEach(l=>l.style.strokeWidth=(1/s)+'px');
  document.querySelectorAll('.places-map-land path').forEach(p=>p.style.strokeWidth=(2/s)+'px');
}
function applyBiblicalPlacesMapTransform(animate=false){
  const svg=document.getElementById('placesMapSvg');if(!svg)return;
  biblicalPlacesMapClamp();
  svg.classList.toggle('map-animate',!!animate);
  svg.style.transform=`translate(${biblicalPlacesMapTranslateX}px,${biblicalPlacesMapTranslateY}px) scale(${biblicalPlacesMapScale})`;
  updateBiblicalPlacesMapReadableScale();
}
function zoomBiblicalPlacesMapAt(factor,clientX,clientY,animate=false){
  const wrap=document.getElementById('placesMapCanvasWrap');if(!wrap)return;
  const r=wrap.getBoundingClientRect(),px=(Number.isFinite(clientX)?clientX:r.left+r.width/2)-r.left,py=(Number.isFinite(clientY)?clientY:r.top+r.height/2)-r.top;
  const old=biblicalPlacesMapScale,next=Math.max(1,Math.min(12,old*factor));if(Math.abs(next-old)<.001)return;
  const mapX=(px-biblicalPlacesMapTranslateX)/old,mapY=(py-biblicalPlacesMapTranslateY)/old;
  biblicalPlacesMapScale=next;biblicalPlacesMapTranslateX=px-mapX*next;biblicalPlacesMapTranslateY=py-mapY*next;
  applyBiblicalPlacesMapTransform(animate);
}
function zoomBiblicalPlacesMap(factor){zoomBiblicalPlacesMapAt(factor,NaN,NaN,true)}
function resetBiblicalPlacesMap(animate=true){
  const wrap=document.getElementById('placesMapCanvasWrap');if(!wrap)return;
  const target=biblicalMapProject(31.7,35.1),rx=target.x/1000*wrap.clientWidth,ry=target.y/560*wrap.clientHeight;
  biblicalPlacesMapScale=2.35;biblicalPlacesMapTranslateX=wrap.clientWidth/2-rx*biblicalPlacesMapScale;biblicalPlacesMapTranslateY=wrap.clientHeight/2-ry*biblicalPlacesMapScale;
  applyBiblicalPlacesMapTransform(animate);
}
function initBiblicalPlacesMapGestures(){
  if(biblicalPlacesMapEventsReady)return;const wrap=document.getElementById('placesMapCanvasWrap');if(!wrap)return;biblicalPlacesMapEventsReady=true;
  const point=e=>({x:e.clientX,y:e.clientY});
  const distance=(a,b)=>Math.hypot(a.x-b.x,a.y-b.y),center=(a,b)=>({x:(a.x+b.x)/2,y:(a.y+b.y)/2});
  wrap.addEventListener('pointerdown',e=>{
    if(e.pointerType==='mouse'&&e.button!==0)return;e.preventDefault();wrap.setPointerCapture?.(e.pointerId);biblicalPlacesMapPointers.set(e.pointerId,point(e));wrap.classList.add('map-dragging');
    if(biblicalPlacesMapPointers.size===1){biblicalPlacesMapGesture={type:'pan',start:point(e),tx:biblicalPlacesMapTranslateX,ty:biblicalPlacesMapTranslateY,moved:false}}
    else if(biblicalPlacesMapPointers.size===2){const [a,b]=[...biblicalPlacesMapPointers.values()],c=center(a,b),r=wrap.getBoundingClientRect();biblicalPlacesMapGesture={type:'pinch',distance:distance(a,b),scale:biblicalPlacesMapScale,mapX:(c.x-r.left-biblicalPlacesMapTranslateX)/biblicalPlacesMapScale,mapY:(c.y-r.top-biblicalPlacesMapTranslateY)/biblicalPlacesMapScale,moved:true}}
  },{passive:false});
  wrap.addEventListener('pointermove',e=>{
    if(!biblicalPlacesMapPointers.has(e.pointerId))return;e.preventDefault();biblicalPlacesMapPointers.set(e.pointerId,point(e));const g=biblicalPlacesMapGesture;if(!g)return;
    if(biblicalPlacesMapPointers.size>=2){const [a,b]=[...biblicalPlacesMapPointers.values()],c=center(a,b),r=wrap.getBoundingClientRect(),next=Math.max(1,Math.min(12,g.scale*distance(a,b)/Math.max(1,g.distance)));biblicalPlacesMapScale=next;biblicalPlacesMapTranslateX=c.x-r.left-g.mapX*next;biblicalPlacesMapTranslateY=c.y-r.top-g.mapY*next;g.moved=true;applyBiblicalPlacesMapTransform(false)}
    else if(g.type==='pan'){const p=point(e),dx=p.x-g.start.x,dy=p.y-g.start.y;if(Math.hypot(dx,dy)>4)g.moved=true;biblicalPlacesMapTranslateX=g.tx+dx;biblicalPlacesMapTranslateY=g.ty+dy;applyBiblicalPlacesMapTransform(false)}
  },{passive:false});
  const end=e=>{
    if(!biblicalPlacesMapPointers.has(e.pointerId))return;const wasMoved=!!biblicalPlacesMapGesture?.moved,last=point(e);biblicalPlacesMapPointers.delete(e.pointerId);try{wrap.releasePointerCapture?.(e.pointerId)}catch(_){}
    if(wasMoved)biblicalPlacesMapSuppressClickUntil=Date.now()+300;
    if(!biblicalPlacesMapPointers.size){wrap.classList.remove('map-dragging');biblicalPlacesMapGesture=null;if(!wasMoved&&e.pointerType==='touch'){const now=Date.now();if(biblicalPlacesMapLastTap&&now-biblicalPlacesMapLastTap.time<320&&Math.hypot(last.x-biblicalPlacesMapLastTap.x,last.y-biblicalPlacesMapLastTap.y)<35){zoomBiblicalPlacesMapAt(1.8,last.x,last.y,true);biblicalPlacesMapLastTap=null;biblicalPlacesMapSuppressClickUntil=now+350}else biblicalPlacesMapLastTap={time:now,x:last.x,y:last.y}}}
    else if(biblicalPlacesMapPointers.size===1){const p=[...biblicalPlacesMapPointers.values()][0];biblicalPlacesMapGesture={type:'pan',start:p,tx:biblicalPlacesMapTranslateX,ty:biblicalPlacesMapTranslateY,moved:true}}
  };
  wrap.addEventListener('pointerup',end,{passive:false});wrap.addEventListener('pointercancel',end,{passive:false});
  wrap.addEventListener('dblclick',e=>{e.preventDefault();zoomBiblicalPlacesMapAt(1.8,e.clientX,e.clientY,true)});
  wrap.addEventListener('wheel',e=>{e.preventDefault();zoomBiblicalPlacesMapAt(e.deltaY<0?1.25:.8,e.clientX,e.clientY,false)},{passive:false});
}
function selectBiblicalPlaceMapMarker(id){if(Date.now()<biblicalPlacesMapSuppressClickUntil)return;activeBiblicalMapPlaceId=id;renderBiblicalPlacesMap();showBiblicalPlacesMapCard(id)}
...
function focusBiblicalPlaceOnMap(id){const x=biblicalPlacesData.find(e=>e.id===id),wrap=document.getElementById('placesMapCanvasWrap');if(!x||!wrap||!biblicalMapHasCoords(x))return;activeBiblicalMapPlaceId=id;document.getElementById('placesMapSearch').value='';document.getElementById('placesMapCategory').value='';biblicalPlacesMapScale=5.2;const p=biblicalMapProject(x.coordinates.latitude,x.coordinates.longitude),px=p.x/1000*wrap.clientWidth,py=p.y/560*wrap.clientHeight;biblicalPlacesMapTranslateX=wrap.clientWidth/2-px*biblicalPlacesMapScale;biblicalPlacesMapTranslateY=wrap.clientHeight/2-py*biblicalPlacesMapScale;renderBiblicalPlacesMap();applyBiblicalPlacesMapTransform(true);showBiblicalPlacesMapCard(id)}
...
function biblicalEntityReturnButton(){return biblicalEntityReturnPosition&&window.__biblicalEntityOpenedFromReader?'<button class="btn soft biblical-return-chapter" type="button" onclick="returnBiblicalEntityToChapter()">Volver al capítulo</button>':''}
...
function ensureBiblicalEntityReturnButton(){
  ...
  for(const target of targets){
    if(target.querySelector('.biblical-return-chapter'))continue;
    const button=document.createElement('button');
    button.className='btn soft biblical-return-chapter';
    button.type='button';
