...
function resetDictionaryDialogLayout(){resetKeyboardAwareDialog(DICTIONARY_DIALOG_LAYOUT)}
...
async function init(){
  ...
  if('caches' in window){
    try{
      ...
      await Promise.all(cacheNames.filter(name=>name.startsWith('biblia-estudio-')&&name!=='biblia-estudio-v2.0.1').map(name=>caches.delete(name)));
...
function toast(message){
  ...
  window._tt=setTimeout(()=>element.classList.add('hidden'),1900);
}
...
document.addEventListener('visibilitychange',()=>{if(!document.hidden)refreshAppStateAfterResume()});
...
/* V2.0.1 · Efecto visual del cambio de capítulo. */
