...
function renderDictionary(query=''){
  ...
  $('#dictionaryResults').innerHTML=visible.length
    ?visible.map(x=>`<button class="dictionary-card" type="button" data-id="${escapeHtml(x.id)}"><strong>${escapeHtml(x.termino)}</strong><small>${escapeHtml(x.categoria||'Sin categoría')} · ${x.resaltar===true?'Cápsula verde':'Solo consulta'}</small><p>${escapeHtml(x.explicacion)}</p></button>`).join('')+(q?'':'<p class="dictionary-live-help">Escribe en el buscador y las coincidencias aparecerán al instante.</p>')
...
const BACKUP_KEYS=[
  ...
  'biblia_personajes_crud_v16433','biblia_parabolas_crud_v16433','biblia_guias_crud_v16435','biblia_profecias_crud_v270',
...
function buildCompleteBackup(){
  ...
  const prophecies=parseBackupStorageValue('biblia_profecias_crud_v270',null);
  return{
    ...
    summary:{
      ...
      propheciesCustom:Array.isArray(prophecies?.custom)?prophecies.custom.length:0,
      propheciesEdited:prophecies?.edits&&typeof prophecies.edits==='object'?Object.keys(prophecies.edits).length:0,
      propheciesDeleted:Array.isArray(prophecies?.deleted)?prophecies.deleted.length:0,
      propheciesTrash:Array.isArray(prophecies?.trash)?prophecies.trash.length:0
    },
    localStorage:data
  };
}
...
function renderBiblicalPlaces(){rebuildBiblicalPlacesData();const list=document.getElementById('biblicalPlacesList');if(!list)return;const q=normalizePlaceText(document.getElementById('biblicalPlacesSearch')?.value);const filtered=biblicalPlacesData.filter(x=>normalizePlaceText([x.name,x.category,x.currentRegion,x.firstAppearance,x.shortDescription,x.biblicalImportance,x.history,x.relatedPeople.join(' '),x.jesusRelation,x.mainVerses.join(' '),x.curiosities.join(' ')].join(' ')).includes(q));document.getElementById('biblicalPlacesCount').textContent=`${filtered.length} ${filtered.length===1?'lugar':'lugares'}`;list.innerHTML=filtered.length?filtered.map(x=>`<button type="button" class="biblical-prophecy-card" onclick="openBiblicalPlaceDetail('${escapeHtml(x.id)}')"><span class="biblical-prophecy-theme">${escapeHtml(x.category||'Lugar bíblico')}</span><strong>${escapeHtml(x.name)}</strong><span class="biblical-prophecy-refs">${escapeHtml(x.currentRegion||x.firstAppearance||'')}</span></button>`).join(''):'<p class="biblical-prophecies-empty">No se han encontrado lugares con esa búsqueda.</p>'}
...
const PREMIUM_PLACE_MAPS={"belen":{"gx":43.2,"gy":68.4,"rx":40.1,"ry":42.0,"generalText":"Belén se encuentra en Judea, al sur de Jerusalén, en la tierra de Israel.","regionalText":"Belén está situada a unos 8 km al sur de Jerusalén, en las colinas de Judea.","precision":"exacta"},"belen-de-juda":{"gx":43.2,"gy":68.4,"rx":40.1,"ry":42.0,"generalText":"Belén de Judá se encuentra en la región de Judea, al sur de Jerusalén.","regionalText":"Belén de Judá corresponde a la ciudad situada a unos 8 km al sur de Jerusalén.","precision":"exacta"},"jerusalen":{"gx":43.4,"gy":66.8,"rx":40.7,"ry":26.3,"generalText":"Jerusalén se encuentra en la zona montañosa de Judea, en el centro de la tierra de Israel.","regionalText":"Jerusalén ocupa las colinas centrales de Judea, al oeste del valle del Jordán y del mar Muerto.","precision":"exacta"},"betania":{"gx":43.7,"gy":66.9,"rx":49.2,"ry":29.6,"generalText":"Betania se encontraba en Judea, muy cerca de Jerusalén.","regionalText":"Betania estaba al este de Jerusalén, en la ladera oriental del monte de los Olivos.","precision":"exacta"},"betfage":{"gx":43.7,"gy":66.8,"rx":46.0,"ry":28.0,"generalText":"Betfagé se encontraba en las cercanías orientales de Jerusalén, dentro de Judea.","regionalText":"Su ubicación se sitúa tradicionalmente entre Jerusalén y Betania, junto al monte de los Olivos.","precision":"aproximada"},"monte-de-los-olivos":{"gx":43.7,"gy":66.7,"rx":45.0,"ry":27.7,"generalText":"El monte de los Olivos se levanta inmediatamente al este de Jerusalén, en Judea.","regionalText":"Está separado de Jerusalén por el valle de Cedrón y se extiende hacia Betania y Betfagé.","precision":"exacta"},"jerico":{"gx":44.0,"gy":67.1,"rx":74.0,"ry":13.2,"generalText":"Jericó se encuentra al este de Judea, cerca del valle del Jordán.","regionalText":"Jericó está situada al noroeste del mar Muerto, en la llanura del Jordán.","precision":"exacta"},"hebron":{"gx":42.9,"gy":70.2,"rx":21.3,"ry":74.9,"generalText":"Hebrón está situada en la zona montañosa del sur de Judea.","regionalText":"Hebrón se encuentra al sur de Belén, en una de las regiones habitadas más antiguas de Judea.","precision":"exacta"},"emaus":{"gx":42.8,"gy":66.8,"rx":21.4,"ry":15.6,"generalText":"Emaús se situaba al oeste de Jerusalén, dentro de la región de Judea.","regionalText":"La localización exacta de Emaús es discutida; el mapa muestra una de las identificaciones tradicionales.","precision":"aproximada"},"en-gadi":{"gx":44.1,"gy":70.7,"rx":68.5,"ry":73.5,"generalText":"En-gadi se encuentra en el desierto de Judea, junto a la costa occidental del mar Muerto.","regionalText":"El oasis de En-gadi está en los acantilados occidentales del mar Muerto, al este de Hebrón.","precision":"exacta"},"tecoa":{"gx":43.4,"gy":69.2,"rx":39.0,"ry":55.0,"generalText":"Tecoa se encontraba en las colinas de Judea, al sur de Belén.","regionalText":"Tecoa se sitúa tradicionalmente al sureste de Belén, en el límite del desierto de Judea.","precision":"aproximada"},"mizpa":{"gx":43.2,"gy":65.8,"rx":35.5,"ry":20.8,"generalText":"Mizpa se encontraba en la zona montañosa al norte de Jerusalén.","regionalText":"Esta Mizpa suele identificarse con un lugar elevado al norte de Jerusalén; la posición es aproximada.","precision":"aproximada"},"adulam":{"gx":42.6,"gy":69.4,"rx":13.5,"ry":57.0,"generalText":"Adulam se encontraba al suroeste de Jerusalén, en la región de Judá.","regionalText":"La ciudad y sus cuevas se sitúan tradicionalmente en las estribaciones occidentales de Judea.","precision":"aproximada"},"beerseba":{"gx":42.3,"gy":74.2,"rx":8.0,"ry":93.0,"generalText":"Beerseba se encontraba en el extremo meridional de la tierra de Judá.","regionalText":"Beerseba queda al sur del área principal mostrada; el marcador señala su dirección aproximada en el borde del mapa.","precision":"aproximada"},"jordan":{"gx":44.2,"gy":65.0,"rx":88.0,"ry":22.0,"generalText":"El río Jordán recorre el valle situado al este de Judea y desemboca en el mar Muerto.","regionalText":"El marcador representa el tramo del Jordán próximo a Jericó; el río continúa hacia el norte fuera de este mapa.","precision":"representativa"}};

function premiumMapOverlay(maps,kind,name){
  const x=kind==="general"?maps.gx:maps.rx;
  const y=kind==="general"?maps.gy:maps.ry;
  return `<span class="premium-map-marker" style="left:${x}%;top:${y}%" aria-hidden="true"><span class="premium-map-pin"></span><span class="premium-map-label">${escapeHtml(name)}</span></span>`;
}

function premiumPlaceMapsBlock(place){
  const maps=PREMIUM_PLACE_MAPS[place?.id];
  if(!maps)return "";
  const name=place.name||"Lugar bíblico";
  const general="mapa-mundo-biblico-base.webp";
  const regional="mapa-judea-base.webp";
  const note=maps.precision==="exacta"?"":`<p class="premium-map-precision">Ubicación ${escapeHtml(maps.precision)}.</p>`;
  return `<section class="place-geography-premium">
    <h2>SITUACIÓN GEOGRÁFICA</h2>
    <article class="place-geography-view">
      <div class="place-geography-copy">
        <h3>Ubicación en el mundo bíblico</h3>
        <p>${escapeHtml(maps.generalText)}</p>
      </div>
      <button type="button" class="place-map-image-button" onclick="openPremiumPlaceMap('${general}','${escapeHtml(name)} · Ubicación en el mundo bíblico',${maps.gx},${maps.gy},'${escapeHtml(name)}')">
        <span class="premium-map-stage">
          <img src="${general}?v=${APP_VERSION}" alt="Mapa general con la ubicación de ${escapeHtml(name)}" loading="lazy">
          ${premiumMapOverlay(maps,"general",name)}
        </span>
      </button>
    </article>
    <article class="place-geography-view">
      <div class="place-geography-copy">
        <h3>Ubicación en la región</h3>
        <p>${escapeHtml(maps.regionalText)}</p>
        ${note}
      </div>
      <button type="button" class="place-map-image-button" onclick="openPremiumPlaceMap('${regional}','${escapeHtml(name)} · Ubicación en la región',${maps.rx},${maps.ry},'${escapeHtml(name)}')">
        <span class="premium-map-stage">
          <img src="${regional}?v=${APP_VERSION}" alt="Mapa regional con la ubicación de ${escapeHtml(name)}" loading="lazy">
          ${premiumMapOverlay(maps,"regional",name)}
        </span>
      </button>
    </article>
  </section>`;
...
let premiumMapDefaultScale=1.28;
let premiumMapZoomState={scale:premiumMapDefaultScale,x:0,y:0,pointers:new Map(),lastDistance:0,lastCenter:null,dragging:false,lastX:0,lastY:0};

function applyPremiumMapTransform(){
  const canvas=document.querySelector("#premiumPlaceMapModal .premium-place-map-canvas");
  if(!canvas)return;
  canvas.style.transform=`translate(${premiumMapZoomState.x}px,${premiumMapZoomState.y}px) scale(${premiumMapZoomState.scale})`;
}
function clampPremiumMapScale(value){return Math.min(5,Math.max(1,value))}
function resetPremiumMapZoom(){
  premiumMapZoomState={scale:premiumMapDefaultScale,x:0,y:0,pointers:new Map(),lastDistance:0,lastCenter:null,dragging:false,lastX:0,lastY:0};
  applyPremiumMapTransform();
}
function zoomPremiumMapBy(factor){
  premiumMapZoomState.scale=clampPremiumMapScale(premiumMapZoomState.scale*factor);
  if(premiumMapZoomState.scale<=premiumMapDefaultScale){premiumMapZoomState.x=0;premiumMapZoomState.y=0}
  applyPremiumMapTransform();
}
function setupPremiumMapGestures(viewport){
  let lastTap=0;
  viewport.addEventListener("pointerdown",e=>{
    viewport.setPointerCapture?.(e.pointerId);
    premiumMapZoomState.pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});
    if(premiumMapZoomState.pointers.size===1){
      premiumMapZoomState.dragging=true;
      premiumMapZoomState.lastX=e.clientX;
      premiumMapZoomState.lastY=e.clientY;
    }else if(premiumMapZoomState.pointers.size===2){
      const pts=[...premiumMapZoomState.pointers.values()];
      premiumMapZoomState.lastDistance=Math.hypot(pts[1].x-pts[0].x,pts[1].y-pts[0].y);
      premiumMapZoomState.lastCenter={x:(pts[0].x+pts[1].x)/2,y:(pts[0].y+pts[1].y)/2};
    }
  });
  viewport.addEventListener("pointermove",e=>{
    if(!premiumMapZoomState.pointers.has(e.pointerId))return;
    premiumMapZoomState.pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});
    if(premiumMapZoomState.pointers.size===2){
      const pts=[...premiumMapZoomState.pointers.values()];
      const distance=Math.hypot(pts[1].x-pts[0].x,pts[1].y-pts[0].y);
      if(premiumMapZoomState.lastDistance>0){
        premiumMapZoomState.scale=clampPremiumMapScale(
          premiumMapZoomState.scale*(distance/premiumMapZoomState.lastDistance)
        );
      }
      const center={x:(pts[0].x+pts[1].x)/2,y:(pts[0].y+pts[1].y)/2};
      if(premiumMapZoomState.lastCenter){
        premiumMapZoomState.x+=center.x-premiumMapZoomState.lastCenter.x;
        premiumMapZoomState.y+=center.y-premiumMapZoomState.lastCenter.y;
      }
      premiumMapZoomState.lastDistance=distance;
      premiumMapZoomState.lastCenter=center;
      applyPremiumMapTransform();
    }else if(premiumMapZoomState.dragging && premiumMapZoomState.scale>1){
      premiumMapZoomState.x+=e.clientX-premiumMapZoomState.lastX;
      premiumMapZoomState.y+=e.clientY-premiumMapZoomState.lastY;
      premiumMapZoomState.lastX=e.clientX;
      premiumMapZoomState.lastY=e.clientY;
      applyPremiumMapTransform();
    }
  ...
  const endPointer=e=>{
    premiumMapZoomState.pointers.delete(e.pointerId);
    premiumMapZoomState.lastDistance=0;
    premiumMapZoomState.lastCenter=null;
    if(premiumMapZoomState.pointers.size===0)premiumMapZoomState.dragging=false;
  };
  viewport.addEventListener("pointerup",endPointer);
  viewport.addEventListener("pointercancel",endPointer);
  viewport.addEventListener("dblclick",e=>{
    e.preventDefault();
    zoomPremiumMapBy(premiumMapZoomState.scale<2?2:0.5);
  });
...
function openPremiumPlaceMap(src,title,markerX=50,markerY=50,markerLabel=''){
  premiumMapDefaultScale=src.includes("mapa-judea-")?1.34:1.24;
  let modal=document.getElementById("premiumPlaceMapModal");
  if(!modal){
    modal=document.createElement("div");
    modal.id="premiumPlaceMapModal";
    modal.className="premium-place-map-modal hidden";
    modal.innerHTML=`<div class="premium-place-map-dialog" role="dialog" aria-modal="true" aria-labelledby="premiumPlaceMapTitle">
      <div class="premium-place-map-head">
        <h2 id="premiumPlaceMapTitle"></h2>
        <button type="button" class="premium-place-map-close" aria-label="Cerrar mapa">×</button>
      </div>
      <div class="premium-place-map-controls" aria-label="Controles del mapa">
        <button type="button" data-map-action="zoom-in" aria-label="Acercar">+</button>
        <button type="button" data-map-action="zoom-out" aria-label="Alejar">−</button>
        <button type="button" data-map-action="reset">Restablecer</button>
      </div>
      <div class="premium-place-map-zoom">
        <div class="premium-place-map-canvas"><img alt="" draggable="false"><span class="premium-map-marker premium-map-marker-modal" aria-hidden="true"><span class="premium-map-pin"></span><span class="premium-map-label"></span></span></div>
      </div>
    </div>`;
    document.body.appendChild(modal);
    const viewport=modal.querySelector(".premium-place-map-zoom");
    setupPremiumMapGestures(viewport);
    modal.querySelector(".premium-place-map-close").addEventListener("click",closePremiumPlaceMap);
    modal.querySelector('[data-map-action="zoom-in"]').addEventListener("click",()=>zoomPremiumMapBy(1.4));
    modal.querySelector('[data-map-action="zoom-out"]').addEventListener("click",()=>zoomPremiumMapBy(1/1.4));
    modal.querySelector('[data-map-action="reset"]').addEventListener("click",resetPremiumMapZoom);
    modal.addEventListener("click",e=>{if(e.target===modal)closePremiumPlaceMap()});
  }
  resetPremiumMapZoom();
  modal.querySelector("#premiumPlaceMapTitle").textContent=title||"Mapa bíblico";
  const img=modal.querySelector("img");
  img.src=src+"?v="+APP_VERSION;
  img.alt=title||"Mapa bíblico";
  const modalMarker=modal.querySelector(".premium-map-marker-modal");
  if(modalMarker){
    modalMarker.style.left=markerX+"%";
    modalMarker.style.top=markerY+"%";
    modalMarker.querySelector(".premium-map-label").textContent=markerLabel||"";
  }
  modal.classList.remove("hidden");
  document.body.classList.add("modal-open");
}
function closePremiumPlaceMap(){
  document.getElementById("premiumPlaceMapModal")?.classList.add("hidden");
  document.body.classList.remove("modal-open");
  resetPremiumMapZoom();
}
window.openPremiumPlaceMap=openPremiumPlaceMap;
window.closePremiumPlaceMap=closePremiumPlaceMap;

function openBiblicalPlaceDetail(id){const x=biblicalPlacesData.find(e=>e.id===id);if(!x)return;activeBiblicalPlaceId=id;document.getElementById('biblicalPlacesHome')?.classList.add('hidden');const d=document.getElementById('biblicalPlaceDetail');if(!d)return;const hasCoords=Number.isFinite(x.coordinates.latitude)&&Number.isFinite(x.coordinates.longitude);const optional=(title,value,cls='biblical-prophecy-explanation')=>String(value||'').trim()?placeSection(title,placeParagraphs(value),cls):'';const optionalList=(title,items)=>items?.length?placeSection(title,placeList(items)):'';d.innerHTML=`<div class="biblical-prophecy-detail-head"><button class="btn soft" type="button" onclick="closeBiblicalPlaceDetail()">← Volver</button>${biblicalEntityReturnButton()}</div><div class="prophecy-detail-tools"><button type="button" onclick="openBiblicalPlaceEditor('${escapeHtml(x.id)}')">Editar</button><button type="button" class="danger" onclick="deleteBiblicalPlace('${escapeHtml(x.id)}')">Eliminar</button></div><p class="biblical-prophecy-detail-theme">${escapeHtml(x.category)}</p><h1>${escapeHtml(x.name)}</h1><img class="biblical-prophecies-divider" src="separador_etiope_transparente_final.png?v=${APP_VERSION}" alt="" aria-hidden="true"><p class="place-current-region">${escapeHtml(x.currentRegion)}</p>${optional('RELACIÓN CON JESÚS',x.jesusRelation,'biblical-prophecy-importance')}${placeSection('DESCRIPCIÓN BREVE',placeParagraphs(x.shortDescription))}${optional('IMPORTANCIA BÍBLICA',x.biblicalImportance,'biblical-prophecy-importance')}${placeSection('PRIMERA APARICIÓN',`<p>${escapeHtml(x.firstAppearance||'Sin especificar')}</p>`,'biblical-prophecy-block place-reference')}${optional('HISTORIA',x.history)}${optionalList('PERSONAJES RELACIONADOS',x.relatedPeople)}${optionalList('VERSÍCULOS PRINCIPALES',x.mainVerses)}${optionalList('CURIOSIDADES',x.curiosities)}${premiumPlaceMapsBlock(x)}<section class="biblical-prophecy-explanation place-map-block"><h2>MAPA BÍBLICO</h2><p>${hasCoords?`${x.coordinates.latitude}, ${x.coordinates.longitude}`:'Coordenadas pendientes'}</p><button type="button" ${hasCoords?'':'disabled'} onclick="showBiblicalPlaceOnMap('${escapeHtml(x.id)}')">Mostrar en el mapa</button></section>`;d.classList.remove('hidden');window.scrollTo({top:0,behavior:'smooth'})}
function closeBiblicalPlaceDetail(){activeBiblicalPlaceId='';document.getElementById('biblicalPlaceDetail')?.classList.add('hidden');document.getElementById('biblicalPlacesHome')?.classList.remove('hidden');window.scrollTo({top:0,behavior:'smooth'})}

let biblicalPlacesLeafletMap=null,biblicalPlacesLeafletClusters=null,biblicalPlacesLeafletMarkers=new Map(),activeBiblicalMapPlaceId='',biblicalPlacesMapFromDetail=false,biblicalPlacesBaseLayer=null,biblicalPlacesMapMode='biblical';
function biblicalMapHasCoords(x){return Number.isFinite(x?.coordinates?.latitude)&&Number.isFinite(x?.coordinates?.longitude)}
function populateBiblicalPlacesMapCategories(){const s=document.getElementById('placesMapCategory');if(!s)return;const current=s.value,cats=[...new Set(biblicalPlacesData.filter(biblicalMapHasCoords).map(x=>x.category).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'es'));s.innerHTML='<option value="">Todas</option>'+cats.map(x=>`<option value="${escapeHtml(x)}">${escapeHtml(x)}</option>`).join('');s.value=cats.includes(current)?current:''}
function biblicalPremiumMarkerIcon(active=false){return L.divIcon({className:'biblical-leaflet-marker-shell',html:`<span class="biblical-leaflet-marker${active?' active':''}"><i></i></span>`,iconSize:[30,38],iconAnchor:[15,36],popupAnchor:[0,-34]})}
function biblicalPremiumClusterIcon(cluster){const n=cluster.getChildCount();return L.divIcon({className:'biblical-cluster-shell',html:`<span class="biblical-cluster"><strong>${n}</strong></span>`,iconSize:[44,44]})}
const BIBLICAL_MAJOR_NAMES=new Set(['Jerusalén','Belén','Nazaret','Capernaúm','Jericó','Betania','Caná','Betsaida','Corazín','Damasco','Antioquía','Egipto','Sinaí','Mar de Galilea','Río Jordán','Judea','Samaria','Galilea','Asia Menor','Roma','Corinto','Éfeso','Cesarea','Cesarea de Filipo','Tiro','Sidón','Hebrón','Beerseba','Siquem','Bet-el','Babilonia','Nínive']);
function biblicalMapTileConfig(mode){
  return mode==='modern'
    ? {url:'https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png',attribution:'© OpenStreetMap contributors © CARTO'}
    : {url:'https://{s}.basemaps.cartocdn.com/light_nolabels/{z}/{x}/{y}{r}.png',attribution:'© OpenStreetMap contributors © CARTO'};
}
function applyBiblicalMapBaseLayer(){
  if(!biblicalPlacesLeafletMap||typeof L==='undefined')return;
  if(biblicalPlacesBaseLayer)biblicalPlacesLeafletMap.removeLayer(biblicalPlacesBaseLayer);
  const cfg=biblicalMapTileConfig(biblicalPlacesMapMode);
  biblicalPlacesBaseLayer=L.tileLayer(cfg.url,{subdomains:'abcd',maxZoom:20,attribution:cfg.attribution,detectRetina:false}).addTo(biblicalPlacesLeafletMap);
}
function updateBiblicalMapModeButtons(){
  document.getElementById('placesMapModeBiblical')?.classList.toggle('active',biblicalPlacesMapMode==='biblical');
  document.getElementById('placesMapModeModern')?.classList.toggle('active',biblicalPlacesMapMode==='modern');
}
function updateBiblicalMarkerLabels(){
  if(!biblicalPlacesLeafletMap)return;
  const zoom=biblicalPlacesLeafletMap.getZoom();
  biblicalPlacesLeafletMarkers.forEach((marker,id)=>{
    const x=biblicalPlacesData.find(e=>e.id===id); if(!x)return;
    const show=biblicalPlacesMapMode==='biblical' && (zoom>=10 || (zoom>=7 && BIBLICAL_MAJOR_NAMES.has(x.name)) || id===activeBiblicalMapPlaceId);
    if(show){
      if(!marker.getTooltip())marker.bindTooltip(x.name,{permanent:true,direction:'top',offset:[0,-30],className:'biblical-place-name-label',opacity:1,interactive:false});
      marker.openTooltip();
...
function setBiblicalMapMode(mode){
  biblicalPlacesMapMode=mode==='modern'?'modern':'biblical';
  try{localStorage.setItem('biblical-map-mode',biblicalPlacesMapMode)}catch(_){ }
  updateBiblicalMapModeButtons();applyBiblicalMapBaseLayer();updateBiblicalMarkerLabels();
}
window.setBiblicalMapMode=setBiblicalMapMode;
function ensureBiblicalLeafletMap(){
  const loading=document.getElementById('placesMapLoading');
  if(typeof L==='undefined'){if(loading){loading.textContent='No se pudo cargar el mapa. Comprueba la conexión a Internet.';loading.classList.remove('hidden')}return false}
  if(biblicalPlacesLeafletMap)return true;
  const el=document.getElementById('placesLeafletMap');if(!el)return false;
  biblicalPlacesLeafletMap=L.map(el,{zoomControl:true,minZoom:3,maxZoom:19,worldCopyJump:true,preferCanvas:true}).setView([31.8,35.15],8);
  try{biblicalPlacesMapMode=localStorage.getItem('biblical-map-mode')||'biblical'}catch(_){biblicalPlacesMapMode='biblical'}
  applyBiblicalMapBaseLayer();updateBiblicalMapModeButtons();
  biblicalPlacesLeafletClusters=L.markerClusterGroup({showCoverageOnHover:false,spiderfyOnMaxZoom:true,removeOutsideVisibleBounds:true,chunkedLoading:true,maxClusterRadius:48,iconCreateFunction:biblicalPremiumClusterIcon});
  biblicalPlacesLeafletMap.addLayer(biblicalPlacesLeafletClusters);
  biblicalPlacesLeafletMap.on('click',()=>{document.getElementById('placesMapCard')?.classList.add('hidden')});
  biblicalPlacesLeafletMap.on('zoomend',updateBiblicalMarkerLabels);
  biblicalPlacesLeafletMap.whenReady(()=>{loading?.classList.add('hidden');setTimeout(()=>biblicalPlacesLeafletMap.invalidateSize(),80)});
  return true;
}
async function openBiblicalPlacesMap(focusId=''){
  ...
  await loadBiblicalPlaces();populateBiblicalPlacesMapCategories();
  biblicalPlacesMapFromDetail=Boolean(focusId);
  if(!ensureBiblicalLeafletMap())return;
  renderBiblicalPlacesMap();
  setTimeout(()=>{biblicalPlacesLeafletMap.invalidateSize();if(focusId)focusBiblicalPlaceOnMap(focusId);else focusBiblicalMapRegion('holyland')},120);
  window.scrollTo({top:0,behavior:'smooth'});
}
function closeBiblicalPlacesMap(){
  ...
  if(biblicalPlacesMapFromDetail&&activeBiblicalPlaceId){document.getElementById('biblicalPlacesHome')?.classList.add('hidden');openBiblicalPlaceDetail(activeBiblicalPlaceId)}else document.getElementById('biblicalPlacesHome')?.classList.remove('hidden');
  biblicalPlacesMapFromDetail=false;window.scrollTo({top:0,behavior:'smooth'});
}
function renderBiblicalPlacesMap(){
  if(!ensureBiblicalLeafletMap())return;
  const q=normalizePlaceText(document.getElementById('placesMapSearch')?.value||''),cat=document.getElementById('placesMapCategory')?.value||'';
  const all=biblicalPlacesData.filter(biblicalMapHasCoords),filtered=all.filter(x=>(!cat||x.category===cat)&&(!q||normalizePlaceText([x.name,x.currentRegion,x.category,x.shortDescription].join(' ')).includes(q)));
  const count=document.getElementById('placesMapCount');if(count)count.textContent=`${filtered.length} visibles · ${biblicalPlacesData.length-all.length} pendientes`;
  biblicalPlacesLeafletClusters.clearLayers();biblicalPlacesLeafletMarkers.clear();
  filtered.forEach(x=>{
    const marker=L.marker([x.coordinates.latitude,x.coordinates.longitude],{icon:biblicalPremiumMarkerIcon(x.id===activeBiblicalMapPlaceId),title:x.name,riseOnHover:true});
    marker.on('click',()=>selectBiblicalPlaceMapMarker(x.id));
    biblicalPlacesLeafletMarkers.set(x.id,marker);biblicalPlacesLeafletClusters.addLayer(marker);
  });
  updateBiblicalMarkerLabels();
  if(q&&filtered.length===1)focusBiblicalPlaceOnMap(filtered[0].id);
  else if(q&&filtered.length>1){const bounds=L.latLngBounds(filtered.map(x=>[x.coordinates.latitude,x.coordinates.longitude]));biblicalPlacesLeafletMap.fitBounds(bounds.pad(.18),{maxZoom:12})}
}
function selectBiblicalPlaceMapMarker(id){
  activeBiblicalMapPlaceId=id;
  biblicalPlacesLeafletMarkers.forEach((m,key)=>m.setIcon(biblicalPremiumMarkerIcon(key===id)));
  showBiblicalPlacesMapCard(id);updateBiblicalMarkerLabels();
}
function showBiblicalPlacesMapCard(id){const x=biblicalPlacesData.find(e=>e.id===id),c=document.getElementById('placesMapCard');if(!x||!c)return;c.innerHTML=`<strong>${escapeHtml(x.name)}</strong><span>${escapeHtml(x.category)} · ${escapeHtml(x.currentRegion)}</span><p>${escapeHtml(x.shortDescription)}</p><button type="button" onclick="openBiblicalPlaceFromMap('${escapeHtml(x.id)}')">Abrir ficha</button>`;c.classList.remove('hidden');c.scrollIntoView({block:'nearest',behavior:'smooth'})}
function focusBiblicalPlaceOnMap(id){const x=biblicalPlacesData.find(e=>e.id===id);if(!x||!biblicalMapHasCoords(x)||!ensureBiblicalLeafletMap())return;activeBiblicalMapPlaceId=id;const marker=biblicalPlacesLeafletMarkers.get(id);biblicalPlacesLeafletMap.setView([x.coordinates.latitude,x.coordinates.longitude],14,{animate:true});setTimeout(()=>{if(marker){biblicalPlacesLeafletClusters.zoomToShowLayer(marker,()=>{selectBiblicalPlaceMapMarker(id)})}else showBiblicalPlacesMapCard(id)},350)}
function focusBiblicalMapRegion(region){if(!ensureBiblicalLeafletMap())return;const regions={holyland:[[29.3,33.8],[33.6,36.7]],egypt:[[21.5,24],[32.2,36.3]],mesopotamia:[[29,36],[38.5,49]],asia:[[35.5,25],[42.5,43]],greece:[[34,7],[44.5,25]]};const b=regions[region]||regions.holyland;biblicalPlacesLeafletMap.fitBounds(b,{padding:[18,18],animate:true})}
function zoomBiblicalPlacesMap(factor){if(!ensureBiblicalLeafletMap())return;biblicalPlacesLeafletMap.setZoom(biblicalPlacesLeafletMap.getZoom()+(factor>1?1:-1))}
function resetBiblicalPlacesMap(){focusBiblicalMapRegion('holyland')}
function openBiblicalPlaceFromMap(id){activeBiblicalPlaceId=id;document.getElementById('biblicalPlacesMap')?.classList.add('hidden');openBiblicalPlaceDetail(id)}
function showBiblicalPlaceOnMap(id){const x=biblicalPlacesData.find(e=>e.id===id);if(!x||!biblicalMapHasCoords(x))return;openBiblicalPlacesMap(id)}
