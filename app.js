...
async function init(){
  ...
  if('caches' in window){
    try{
      ...
      await Promise.all(cacheNames.filter(name=>name.startsWith('biblia-estudio-')&&name!=='biblia-estudio-v1.39.0').map(name=>caches.delete(name)));
...
(function activarGestosPanelAcciones(){
  ...
  window.addEventListener('touchstart',e=>{
    if(e.touches.length!==1||state.selected.size===0||panel.classList.contains('open'))return;
