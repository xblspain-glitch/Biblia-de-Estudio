...
async function init(){
  ...
  if('caches' in window){
    try{
      ...
      await Promise.all(cacheNames.filter(name=>name.startsWith('biblia-estudio-')&&name!=='biblia-estudio-v1.64.71').map(name=>caches.delete(name)));
...
async function openDailyVerse(){
  const v=await ensureDailyVerse();paintDailyVerse(v);$('#dailyVerseDialog').showModal();localStorage.setItem('dailyVerseLastSeen',v.dateKey);
}
...
async function initializeDailyVerse(){
  ...
  if(forced||localStorage.getItem('dailyVerseLastSeen')!==v.dateKey)setTimeout(()=>openDailyVerse(),450);
}
