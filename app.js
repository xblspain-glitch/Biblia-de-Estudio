...
function updateDictionaryDialogViewport(){
  const dialog=$('#dictionaryDialog');
  if(!dialog?.open||!dialog.classList.contains('dictionary-live-layout'))return;
  const vv=window.visualViewport;
  const visibleHeight=vv?.height||window.innerHeight;
  const visibleTop=vv?.offsetTop||0;
  dialog.style.setProperty('--dictionary-vv-height',`${visibleHeight}px`);
  dialog.style.setProperty('--dictionary-vv-top',`${visibleTop}px`);
  const inputFocused=document.activeElement===$('#dictionarySearch');
  const keyboardVisible=visibleHeight < window.innerHeight*0.82;
  dialog.classList.toggle('keyboard-visible',inputFocused||keyboardVisible);
}
function resetDictionaryDialogLayout(){
  const dialog=$('#dictionaryDialog');
  dialog?.classList.remove('dictionary-live-layout','keyboard-visible');
  dialog?.style.removeProperty('--dictionary-vv-height');
  dialog?.style.removeProperty('--dictionary-vv-top');
}
...
async function init(){
  ...
  if('caches' in window){
    try{
      ...
      await Promise.all(cacheNames.filter(name=>name.startsWith('biblia-estudio-')&&name!=='biblia-estudio-v2.0.0').map(name=>caches.delete(name)));
...
function openDictionary(initialQuery=''){
  ...
  setTimeout(()=>{const input=$('#dictionarySearch');input?.focus();if(query)input?.select();updateDictionaryDialogViewport()},80);
}
...
$('#dictionarySearch')?.addEventListener('focus',()=>requestAnimationFrame(updateDictionaryDialogViewport));
$('#dictionarySearch')?.addEventListener('input',()=>requestAnimationFrame(updateDictionaryDialogViewport));
$('#dictionarySearch')?.addEventListener('blur',()=>setTimeout(updateDictionaryDialogViewport,80));
$('#dictionaryDialog')?.addEventListener('close',resetDictionaryDialogLayout);
window.visualViewport?.addEventListener('resize',updateDictionaryDialogViewport);
window.visualViewport?.addEventListener('scroll',updateDictionaryDialogViewport);
window.addEventListener('orientationchange',()=>setTimeout(updateDictionaryDialogViewport,120));
...
/* V2.0.0 · Efecto visual del cambio de capítulo, antes incrustado en index.html. */
