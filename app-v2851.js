...
function placeMapLaunchersBlock(place){
  ...
  return `<section class="place-geography-premium place-map-choice-section">
    <h2>SITUACIÓN GEOGRÁFICA</h2>
    <p class="place-map-choice-intro">Elija cómo desea situar ${name}. Ambos mapas se abren a pantalla grande y permiten ampliar, mover y consultar los lugares cercanos.</p>
    <div class="place-map-choice-grid">
      <button type="button" class="place-map-choice-card" onclick="openBiblicalPlaceMapMode('${id}','biblical')">
        <span class="place-map-choice-title">Mapa bíblico</span>
        <span class="place-map-choice-description">Vista limpia con los nombres de los lugares bíblicos y marcadores cercanos.</span>
        <span class="place-map-choice-action">Abrir y ampliar</span>
      </button>
      <button type="button" class="place-map-choice-card" onclick="openBiblicalPlaceMapMode('${id}','modern')">
        <span class="place-map-choice-title">Mapa moderno</span>
        <span class="place-map-choice-description">Cartografía actual para reconocer ciudades, carreteras y territorios modernos.</span>
        <span class="place-map-choice-action">Abrir y ampliar</span>
      </button>
    </div>
  </section>`;
...
async function openBiblicalPlaceMapMode(placeId,mode){
  biblicalPlacesMapMode=mode==="modern"?"modern":"biblical";
  try{localStorage.setItem("biblical-map-mode",biblicalPlacesMapMode)}catch(_){}
  await openBiblicalPlacesMap(placeId);
  setTimeout(()=>{
    setBiblicalMapMode(biblicalPlacesMapMode);
    focusBiblicalPlaceOnMap(placeId);
    biblicalPlacesLeafletMap?.invalidateSize();
  },180);
}
window.openBiblicalPlaceMapMode=openBiblicalPlaceMapMode;
