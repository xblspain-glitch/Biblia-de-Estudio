...
function ensureBiblicalLeafletMaps(){
  ...
  if(!biblicalPlacesBiblicalMap){
    ...
    biblicalPlacesBiblicalMap.on('zoomend',updateBiblicalMarkerLabels);
  }
...
function updateBiblicalMarkerLabels(){
  ...
  biblicalPlacesBiblicalMarkers.forEach((marker,id)=>{
    const x=biblicalPlacesData.find(e=>e.id===id);
    if(!x)return;
    const show=zoom>=10||(zoom>=7&&BIBLICAL_MAJOR_NAMES.has(x.name))||id===activeBiblicalMapPlaceId;
    if(show){
      if(!marker.getTooltip()){
        marker.bindTooltip(x.name,{
          ...
          direction:'top',
          offset:[0,-30],
          className:'biblical-place-name-label',
          opacity:1,
          interactive:false
        });
      }
